...
`agents/supervisor/graph.py`는 각 에이전트의 URL을 독립적으로 해석합니다 — 공유 `REMOTEGRAPH_BASE_URL` 대신 `RESEARCHER_URL`/`CODER_URL`/`REVIEWER_URL`을 설정하면 supervisor가 별도로 호스팅된 서버들을 가리키도록 할 수 있습니다. 에이전트별 환경변수는 항상 `REMOTEGRAPH_BASE_URL`보다 우선하며, `REMOTEGRAPH_BASE_URL`은 단일 백엔드 사용 시 fallback으로 남습니다.
...
별도 서버를 실제로 띄우는 CLI 경로는 두 가지입니다 — 에이전트 뒤에 얼마나 많은 백엔드를 둘지에 따라 선택합니다:

**A. 에이전트마다 완전한 Aegra 스택** — 성숙한 백엔드(FastAPI + Postgres + Redis)를 재사용하므로 에이전트마다 영속성/스레드/스토어를 그대로 얻습니다. 각 에이전트는 격리된 docker compose 프로젝트로 각자의 포트에서 실행됩니다:

```bash
uv run remotegraph agent deploy-distributed   # researcher:2027, coder:2028, reviewer:2029
uv run remotegraph agent urls                 # export RESEARCHER_URL=... 라인을 출력
uv run remotegraph agent stop-distributed
```

**B. 에이전트마다 독립 FastAPI 마이크로서비스** — `docker/agent-server/`가 [`src/remotegraph/agent_server.py`](src/remotegraph/agent_server.py)를 실행합니다. `RemoteGraph`가 필요로 하는 LangGraph Platform API의 일부(`POST /runs/stream`)만 구현한 무상태 단일-그래프 서버입니다. **Postgres/Redis 불필요** — 가장 가벼운 "에이전트 = 마이크로서비스" 옵션:

```bash
uv run remotegraph agent up-servers           # researcher:8101, coder:8102, reviewer:8103
uv run remotegraph agent down-servers
# 또는 Docker 없이 로컬에서 하나만:
...
# 분산: 에이전트마다 별도 서버 ("분산 배포" 섹션 참고)
remotegraph agent deploy-distributed|stop-distributed|urls   # A: 에이전트마다 Aegra 스택
remotegraph agent up-servers|down-servers                    # B: 에이전트마다 독립 FastAPI
remotegraph agent serve <name> [--port <p>]                  # B: FastAPI 서버 1개, Docker 없이
```
...
다양한 프롬프트 시나리오(단일 호출, 멀티턴 스레드 상태, 스트리밍, 서브그래프 분기 제어, 에러 처리, 멀티에이전트 파이프라인)를 실제 응답과 함께 실증한 문서는 [`docs/experiments-ko.md`](docs/experiments-ko.md) (한국어) / [`docs/experiments-en.md`](docs/experiments-en.md) (English)에 있으며, [`scripts/run_experiments.py`](scripts/run_experiments.py)로 재현 가능합니다.

## 프로젝트 구조

```
src/remotegraph/      # CLI (typer): cli.py, config.py, host.py, agent.py, backends/, workflow.py (YAML 로더)
src/remotegraph/agent_server.py  # 독립 단일-에이전트 FastAPI 서버 (분산 배포 "B")
src/remotegraph/distributed.py   # 에이전트마다 Aegra 스택 오케스트레이션 (분산 배포 "A")
...
docker/aegra/          # Dockerfile + docker-compose.yml + aegra.json (+ "A"용 docker-compose.distributed.yml)
docker/open-langgraph/ # Dockerfile + docker-compose.yml + open_langgraph.json
docker/agent-server/   # 독립 FastAPI 에이전트 서버용 Dockerfile + docker-compose.yml ("B")
