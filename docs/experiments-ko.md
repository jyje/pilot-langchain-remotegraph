...
가장 기본적인 시나리오: 원격 그래프에 메시지 하나를 보내고 답을 받는다. 호출 대상은 `coder` — ReAct 패턴의 그래프로, `researcher`/`reviewer`도 동일한 구조다.
...
→ `RemoteGraph.invoke()`가 한국어 메시지를 그대로 전달하고, 원격 `coder` 에이전트가 정확히 답했다. 이번 호출은 도구를 쓰지 않고 `__start__ → model → __end__`로 바로 끝났다.

## 2. 멀티턴 스레드 상태 유지 (Thread persistence)

같은 `thread_id`로 두 번 호출해, 원격 서버가 대화 맥락을 실제로 기억하는지 확인. 그래프 토폴로지는 1번과 동일한 `coder`이고, 두 번의 `invoke()`가 같은 `thread_id`로 `__start__`부터 다시 실행된다 — 상태는 그래프 안이 아니라 서버의 스레드 저장소에 남는다.
...
`client.runs.stream(..., stream_mode="values")`로 중간 이벤트가 실제로 여러 번 오는지 확인. 역시 `coder` 그래프를 그대로 쓰며, `stream_mode="values"`는 `__start__`부터 `__end__`까지 지나는 동안의 각 상태 스냅샷을 이벤트로 내보낸다.
...
`subgraph_demo` (`prepare → inner(subgraph: validate → {transform → format_text | reject}) → finalize`)에 입력을 다르게 주어 분기를 실제로 타게 함. `inner` 노드 자체가 컴파일된 서브그래프라서, `xray=True`로 펼치면 그 안의 `validate`/`transform`/`format_text`/`reject` 노드와 조건 분기가 그대로 보인다.
...
아래 표의 두 입력은 각각 `validate`의 `ok`/`invalid` 분기를 실제로 통과한다 — `__start__ → prepare → inner:validate → (ok면 inner:transform → inner:format_text / invalid면 inner:reject) → inner:__end__ → finalize → __end__`.
...
존재하지 않는 그래프 이름(`does-not-exist`)으로 호출했을 때, 클라이언트가 실패를 어떻게 전달하는지 확인. 이 시나리오는 `__start__`조차 실행되지 않는다 — 서버가 assistant 이름을 그래프 목록과 대조하는 단계에서 즉시 422로 막기 때문에, 보여줄 그래프 토폴로지가 없는 것 자체가 핵심 확인 포인트다.
...
위 그래프는 슈퍼바이저 **로컬** 그래프의 토폴로지다. `research`/`code`/`review` 세 노드는 각각 내부에서 `RemoteGraph("researcher"/"coder"/"reviewer", url=...).invoke(...)`를 호출하는데, 그 각 원격 호출이 다시 1번 시나리오와 동일한 `__start__ → model ⇄ tools → __end__` 그래프를 처음부터 끝까지 실행한다. 즉 실제로는 4개의 그래프(슈퍼바이저 1개 + 원격 ReAct 그래프 3개)가 `__start__`부터 각자 독립적으로 실행된다.
